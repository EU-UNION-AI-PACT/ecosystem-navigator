...
export default function ClusterDetail() {
  ...
  const countries = useMemo(() => cluster ? [...new Set(cluster.partners.map((p) => p.country).filter(Boolean))].sort() : [], [cluster]);
  const allIntegrations = useMemo(() => cluster ? [...new Set(cluster.partners.flatMap((p) => p.integration?.split(", ") || []))].sort() : [], [cluster]);
  const statuses = useMemo(() => cluster ? [...new Set(cluster.partners.map((p) => p.status || "active"))] : [], [cluster]);
  ...
  const filteredPartners = useMemo(() => {
    ...
    return cluster.partners.filter((p) => {
      ...
      const matchesStatus = !statusFilter || (p.status || "active") === statusFilter;
      const matchesCountry = !countryFilter || p.country === countryFilter;
      const matchesIntegration = !integrationFilter || p.integration?.includes(integrationFilter);
      return matchesSearch && matchesStatus && matchesCountry && matchesIntegration;
    });
  }, [cluster, searchQuery, statusFilter, countryFilter, integrationFilter]);

  const activeCount = cluster ? cluster.partners.filter((p) => p.status === "active").length : 0;
  const reviewCount = cluster ? cluster.partners.filter((p) => p.status === "review").length : 0;
  const memberCount = cluster ? cluster.partners.filter((p) => p.status === "membership").length : 0;

  const hasActiveFilters = statusFilter || countryFilter || integrationFilter || searchQuery;
